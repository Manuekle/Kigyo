...
const XLSX_DROPDOWN_Z_INDEX_CLASS = "z-[200]"
...
function WorkbookFileActionsMenu({
  ...
  // Kigyo: el menú de 3 puntos no se usa en la vista previa (sin descarga,
  // sin subida; el tema lo manda la app). Se conserva la firma por compat.
  return null
}
...
function WorkbookSearchPopover({
  ...
  return (
    <Popover>
      ...
      <PopoverContent align="end" className="w-64 gap-2 rounded-xl p-2">
        ...
        <div className="flex items-center justify-between gap-2 px-1 text-xs tabular-nums text-muted-foreground">
          ...
          {activeResult ? (
            <span className="truncate font-mono text-[11px]">
              {activeResult.sheetName}
              {cellAddressToA1(activeResult.cell)}
            </span>
          ) : null}
        </div>
      </PopoverContent>
  ...
  return (
    ...
    >
      ...
      previewUrl
        ? createPortal(
            <div
              className="pointer-events-none fixed z-[200] translate-y-0 overflow-hidden rounded-lg border bg-background/95 opacity-100 shadow-xl backdrop-blur-md transition-[opacity,transform] duration-100"
              style={{
