...
export type PDFViewerProps = {
  ...
  showUpload?: boolean
  src?: string
  /** Preferido vs signed URL: buffer ya descargado (evita fetch blob:/Range en worker). */
  buffer?: ArrayBuffer
        ...
        <div className="flex min-w-0 flex-wrap items-center justify-end gap-1">
          ...
          <div className="flex flex-none items-center gap-1">
            ...
            <Select
              ...
              disabled={controlsDisabled}
              ariaLabel="Nivel de zoom"
              options={ZOOM_OPTIONS.map((option) => ({
                value: String(option),
...
function PDFViewerScroller({
  ...
  // First paint: con layout recién computado los tiles aún no existen, y sin
  // un evento externo (zoom/scroll) no se generan — el PDF se ve en blanco.
  // Refrescar todas las páginas un frame después del primer layout fuerza ese
  // pase inicial de render.
  const { registry } = useRegistry()
  const refreshedDocumentRef = React.useRef<string | null>(null)
  React.useEffect(() => {
    if (!scrollerLayout || !registry) return
    if (refreshedDocumentRef.current === documentId) return

    const frame = window.requestAnimationFrame(() => {
      if (refreshedDocumentRef.current === documentId) return
      refreshedDocumentRef.current = documentId
      registry.getStore().dispatchToCore(
        refreshPages(
          documentId,
          scrollerLayout.items.flatMap((item) =>
            item.pageLayouts.map((page) => page.pageIndex)
          )
        )
      )
    })
    return () => window.cancelAnimationFrame(frame)
  }, [documentId, registry, scrollerLayout])
...
function PDFViewerInner({
  ...
  // First paint fix: el zoom inicial corre antes de que Scroll/Tiling
  // computen el layout; el refresco real de páginas vive en
  // `PDFViewerScroller`, que se dispara cuando el layout ya existe.
...
function PDFViewerDocumentLoader({
  ...
  const openKey = pdfBuffer
    ? `buffer:${fileName ?? "document.pdf"}:${pdfBuffer.byteLength}`
    : pdfFile
  ...
  React.useEffect(() => {
    if (!documentManager || (!pdfFile && !pdfBuffer)) return
    if (openedFileRef.current === openKey) return

    openedFileRef.current = openKey
    setLoadError(false)
    ...
    const handleOpenError = () => {
      if (openedFileRef.current === openKey) setLoadError(true)
    }

    const openTask = pdfBuffer
      ? documentManager.openDocumentBuffer({
          buffer: pdfBuffer.slice(0),
          name: fileName?.trim() || "document.pdf",
          autoActivate: true,
        })
  ...
  }, [documentManager, fileName, openKey, pdfBuffer, pdfFile])

  const document =
    activeDocument?.status === "loaded" ? activeDocument.document : null
  const documentFailed = loadError || activeDocument?.status === "error"
  const hasSource = Boolean(pdfBuffer || pdfFile)

  if (!activeDocumentId || documentFailed || !hasSource) {
    return (
      <PDFViewerFallbackShell
        ...
        sidebarOpen={false}
        state={!hasSource ? "empty" : documentFailed ? "error" : "loading"}
  ...
  return (
    <PDFViewerInner
      ...
      {...innerProps}
      fileName={fileName}
      pdfFile={pdfFile || fileName || "document.pdf"}
...
export const PDFViewer = React.forwardRef<PDFViewerHandle, PDFViewerProps>(
  function PDFViewer(
    ...
    const pdfFile = buffer ? "" : uploadedPdfUrl ?? src ?? ""
