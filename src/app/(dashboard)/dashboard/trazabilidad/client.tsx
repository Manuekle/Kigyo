...
export default function TrazabilidadPage() {
  ...
  const exportLog = () => {
    ...
    void runExport(rows, 'trazabilidad-kigyo', 'trazabilidad')
  }
  return (
    <div className="card rise d1">
      <div className="chead">
        <TabBar value={f} onChange={setF} items={chips.map((c) => ({ key: c, label: c }))} />
        <button disabled={exporting} aria-busy={exporting} className="btn" data-tip="Exportar a Excel" onClick={exportLog}><FileSpreadsheet size={14} />Exportar</button>
      </div>
      <div className="tl">
        {EVENTOS.map((grp) => {
