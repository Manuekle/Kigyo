...
Próximo paso: decisión abierta. Opciones:
  ...
  (b) Retomar RAG: probar calidad híbrida con docs reales, ajustar umbral 0.68, o pipeline de ingestión de archivos nuevos. → CERRADO. Ingestión (PDF/docx/xlsx) HECHO + umbral calibrado 0.68 → 0.60 con benchmark real (tabla en commit; pregunta genérica ya no devuelve vacío). Re-indexar docs existentes queda como acción de usuario en UI (botón ya existe).
